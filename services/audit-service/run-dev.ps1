#!/usr/bin/env pwsh
# SecureCloud Audit Service Development Launcher
# Loads environment variables and starts the audit service
# Run from project root: .\services\audit-service\run-dev.ps1
...
$BinaryPath = "build\dev\services\audit-service\audit-service.exe"
$EnvFile = "config\env\$Environment\.env"
$WorkingDir = "services\audit-service"

Write-Host "=== SecureCloud $ServiceName Dev Launcher ===" -ForegroundColor Cyan
Write-Host "Environment: $Environment" -ForegroundColor Gray

if ($Help) {
    Write-Host "Usage: .\services\audit-service\run-dev.ps1 [-Environment <dev|prod>] [-Help]"
    Write-Host "  -Environment  Environment to use (default: dev)"
    Write-Host "  -Help         Show this help message"
    exit 0
}

# Detect project root (script is in services/audit-service/)
...
try {
    ...
    if (!(Test-Path $BinaryPath)) {
        ...
        Write-Host "Build first: cmake --build build/dev --target audit-service" -ForegroundColor Yellow
        exit 1
    }

#!/usr/bin/env pwsh
# SecureCloud Messaging Service Development Launcher
# Loads environment variables and starts the messaging service
# Run from project root: .\services\messaging-service\run-dev.ps1
...
$BinaryPath = "build\dev\services\messaging-service\messaging-service.exe"
$EnvFile = "config\env\$Environment\.env"
$WorkingDir = "services\messaging-service"

Write-Host "=== SecureCloud $ServiceName Dev Launcher ===" -ForegroundColor Cyan
Write-Host "Environment: $Environment" -ForegroundColor Gray

if ($Help) {
    Write-Host "Usage: .\services\messaging-service\run-dev.ps1 [-Environment <dev|prod>] [-Help]"
    Write-Host "  -Environment  Environment to use (default: dev)"
    Write-Host "  -Help         Show this help message"
    exit 0
}

# Detect project root (script is in services/messaging-service/)
...
try {
    ...
    if (!(Test-Path $BinaryPath)) {
        ...
        Write-Host "Build first: cmake --build build/dev --target messaging-service" -ForegroundColor Yellow
        exit 1
    }

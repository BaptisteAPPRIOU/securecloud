#!/usr/bin/env pwsh
# SecureCloud Files Service Development Launcher
# Loads environment variables and starts the files service
# Run from project root: .\services\files-service\run-dev.ps1
...
$BinaryPath = "build\dev\services\files-service\files-service.exe"
$EnvFile = "config\env\$Environment\.env"
$WorkingDir = "services\files-service"

Write-Host "=== SecureCloud $ServiceName Dev Launcher ===" -ForegroundColor Cyan
Write-Host "Environment: $Environment" -ForegroundColor Gray

if ($Help) {
    Write-Host "Usage: .\services\files-service\run-dev.ps1 [-Environment <dev|prod>] [-Help]"
    Write-Host "  -Environment  Environment to use (default: dev)"
    Write-Host "  -Help         Show this help message"
    exit 0
}

# Detect project root (script is in services/files-service/)
...
try {
    ...
    if (!(Test-Path $BinaryPath)) {
        ...
        Write-Host "Build first: cmake --build build/dev --target files-service" -ForegroundColor Yellow
        exit 1
    }
